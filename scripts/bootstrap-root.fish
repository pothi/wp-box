...
if false
    ...
    check_result $status "Unable to fetch upstream vimrc changes."
    # cat $remote_vimrc
...
if test $(vim --version | grep .config/vim/vimrc)
    ...
    check_result $status "Could not create symlink from $current_vimrc to ~/.vimrc"
end
...
if not test -f $viminfo_config
    ...
    check_result $status "Unable to download viminfo config file."
end
...
if false
    ...
    check_result $status "Unable to fetch upstream editorconfig changes."
